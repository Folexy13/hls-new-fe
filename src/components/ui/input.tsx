  ...
  ({ className, type, onFocus, ...props }, forwardedRef) => {
    ...
    return (
      <input
        type={type}
        className={cn(
          "flex h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 shadow-sm file:border-0 file:bg-transparent file:text-sm file:text-foreground placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-500 md:text-sm",
          className
        )}
        ref={setRef}
        onFocus={handleFocus}
